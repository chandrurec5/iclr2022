...
(i) Learning in the gates is the most crucial for finite width networks to outperform infinite width NTK; this addresses \textbf{Issue I}.

(ii) In the infinite width regime, when learning the weights with fixed gates, it was shown that the NTK is equal to (but for a scalar) a so called neural path kernel (NPK) which is a kernel solely based on inputs and gates and is dictated by the gating network. In other words, the role of the weight network is to realise the NPK, and now \textbf{Issue II} related to interpretability gets shifted to the gating network, i.e., to interpreting the gates as opposed to interpreting the gradients.
