...
$\bullet$ \textbf{Disentangling Value Network.} While there is entanglement of GaLUs and the linear operations,  the value network computes not layer-by-layer, instead it computes path-by-path : the value network is \textbf{dual linear}. Therefore the disentanglement happens in the path variables. Dual linearity has a kernel interpretation in the \emph{infinite width regime}  (see below discussion on dual linearity).
...
\textbf{Dual Linearity (Experiments).}  In \Cref{sec:exp}, we show that the value network does not learn in a layer-by-layer manner; this is important to convince ourselves that the value network is indeed `dual linear' and computes path-by-path.
