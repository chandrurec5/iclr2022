...
\textbf{Contribution I.} In the first part of the paper, we look at the issues of entanglement and `black box'-ness. For this, we turn towards the DGN, since it is only marginally poor compared to the DNN; by successfully addressing the `black box'-ness issue in a DGN we will have both performance and interpretability. In a DGN, there are two sources of entanglements, one in the feature network and the other in the value network. We propose a novel \texttt{DGN-NO-ACT} obtained by modifying the feature network of the DGN to address the issues of entanglement and `black box'-ness. The key points are:
