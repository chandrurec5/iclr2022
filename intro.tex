...
$\bullet$ \textbf{Dual View} (\Cref{sec:analysis}).  We present an unnoticed insight in prior work on fully connected networks that the \textbf{NPK is a product kernel} and is invariant to layer permutations. We present new results to show that (i) the \textbf{NPK is rotationally invariant} for convolutional networks with global average pooling, and (ii) the \textbf{NPK is an ensemble of many kernels} in the presence of skip connections. 
