...
Prior works \citep{ntk,arora2019exact,cao2019generalization} showed that training an infinite width DNN with gradient descent is equivalent to a kernel method with the so called \emph{neural tangent kernel} matrix. While as a pure kernel method, the NTK matrix performed better than other pure kernel methods, it was an open question in NTK theory to understand why finite width DNNs still outperformed their corresponding NTK \citep{arora2019exact}. %The open question with the NTK theory was to explain why finite width DNNs are better than infinite width NTK \citep{arora2019exact}. 
