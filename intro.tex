...
The dual view breaks down the computations path-by-path. Consider as simple toy network (\Cref{}) with an input node, $2$ layer and $2$ hidden units in each layer and an output node. Let the input be $x\in\R$ and the output be $\hat{y}$. This network has $4$ gates (i.e., ReLUs) and $4$ paths. Let us say for this particular input $x$, the bottom ReLU/gate in the first layer is `off' and rest of the ReLUs are `on'. In this case, paths $p_1$ and $p_2$ are `on' and paths $p_3$ and $p_4$ are `off'.  The product of the weights in each path is encoded in the so called neural path value $v=(0.1 \cdot 0.2 \cdot 0.7,0.1 \cdot 0.3 \cdot 0.6,-0.1 \cdot 0.4 \cdot 0.7,-0.1 \cdot 0.5 \cdot 0.6)$.  The input dependent information is encoded in the so called neural path feature $\phi(x)= (x\cdot 1, x\cdot 1, x\cdot 0, x\codt 0)= (x,x,0,0)$, wherein, the coordinates corresponding to `on' paths $p_1$ and $p_2$ the signal $x$ at the input node and the coordinates corresponding to `off' paths $p_3$ and $p_4$ are $0$. The output is then the inner product of the neural path feature and neural path value, i.e., $\hat{y}=\ip{\phi(x),v}$.
...
\textbf{Contribution I.} In a DGN, there are two sources of entanglements, one in the feature network and the other in the value network. We propose a novel \texttt{DGN-NO-ACT} obtained by modifying the feature network of the DGN to address the issues of entanglement and `black box'-ness. The key points are:
