...
$\bullet$ \textbf{Extending Dual View}(\Cref{sec:analysis}).  We present an unnoticed insight in prior work on fully connected networks that the \textbf{NPK is a product kernel} and is invariant to layer permutations. We present new results  that (i) the \textbf{NPK is rotationally invariant} in the presence of convolutions with global average pooling, and (i) the \textbf{NPK is an ensemble of many kernels} in the presence of skip connections. 
