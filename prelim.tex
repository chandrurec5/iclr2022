\section{Prior Work : Dual View For DNNs with ReLUs}\label{sec:prelim}
...
\subsection{Experimental Results: External Gating Fixed and Learnable}
\begin{figure}
\centering
\begin{minipage}{0.8\columnwidth}
\centering
...
{\centering \emph{{Given the learnt gates, i.e., learnt features $\phi_{\Tf}(x)$s can we learn weights, i.e., $v_{\Tv}$?}}\par}
...
{\centering \emph{{In a DNN, are gates i.e., $\phi_{\Theta}(x)$s learnt during training? If so, how important is such learning?}}\par}
...
{\centering \emph{{Can gates, i.e., $\phi_{\Tf}(x)$s and weights, i.e., $v_{\Tv}$ be learnt simultaneously? or How good is a DGN?}}\par}
...
\subsection{Theoretical Result: External Fixed Gates}
