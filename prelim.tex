...
Since the feature and value networks have identical architecture, there is an one-to-one correspondence between the ReLUs and GaLUs in the respective networks.  The feature network has ReLUs which turn `on/off' based on their pre-activation signal, and the value  network has gated linear units (GaLUs) \citep{sss,npk}, which multiply their respective pre-activation inputs by the external gating signals provided by the feature network.  Feature network realises $\phi_{\Tf}(x)$ by turning the gates `on/off' the correpsonding GaLUs in the value network. The value network realises $v_{\Tv}$ and computes the output $\hat{y}_{\text{DGN}}(x)=\ip{\phi_{\Tf}(x),v_{\Tv}}$.
