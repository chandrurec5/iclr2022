...
We start with describing the encoding of gates, weights and subnetworks, followed by a description of deep gated network setup which separates the gates from the weights, and then we present theoretical results on learning the weights with fixed gates and the experimental results on learning in the gates.
\begin{comment}
\begin{figure}[t]
...
\textbf{Deep Gated Network (DGN)} is a setup to separate the gates form the weights. Consider a DNN with ReLUs with weights $\Theta\in\R^{\dnet}$. The DGN \emph{corresponding} to this DNN (see the left diagram in \Cref{fig:dgn}) has two networks of \emph{identical archicture} (to the DNN) namely the gating/feature network and weight/value network with distinct weights $\Tf\in\R^{\dnet}$ and $\Tv\in\R^{\dnet}$ respectively. %The main difference between the feature and value networks is in the activations. 
The feature network has ReLUs which turn `on/off' based on their pre-activation signal, and the value  network has gated linear units (GaLUs) \citep{sss,npk}, which multiply their respective pre-activation inputs by an external gating signals provided by the feature network. Since the feature and value networks have identical architecture, there is an one-to-one correspondance between the ReLUs and GaLUs in the respective networks. Feature network realises $\phi_{\Tf}(x) by turning the gates `on/off' the correpsonding GaLUs in the value network. The value network realises $v_{\Tv}$ and computes the output $\hat{y}_{\text{DGN}}(x)=\ip{\phi_{\Tf}(x),v_{\Tv}}$.
...
During training a DNN learns both $\phi_{\Theta}(x)$ as well as $v_{\Theta}$ simultaneously, and a finite time characterisation this learning in finite width DNNs is desirable. However, this is a hard problem. A slightly easier problem is to understand in a DGN, how the weights in the value network are learnt when the gates are fixed in the feature network, i.e., the relation $\hat{y}_{DGN}(x)=\ip{\phi_{\Tf}(x),v_{\Tv}}$ is learnt by learning $v_{\Tv}$ with fixed $\phi_{\Tf}(x)$. This is a linear model in dual variables, which we call as \textbf{dual linearity}. \cite{npk} showed that the dual linearity is characterised by the NPK. We state the assumption followed by Theorem 5.1 in \citep{npk}.
...
\subsubsection{Learning in Gates : Why Finite Width Is Better Than Infinite Width} 
