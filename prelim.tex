...
The gating network has ReLUs which turn `on/off' based on their pre-activation signal, and the weight network has gated linear units (GaLUs) \citep{sss,npk}, which multiply their respective pre-activation inputs by the external gating signals provided by the feature network.  Since the gating and weight networks have identical architecture, there is a one-to-one correspondence between the ReLUs and GaLUs in the respective networks.  Gating network realises $\phi_{\Tf}(x)$ by turning the gates `on/off' the corresponding GaLUs in the weight network. The value network realises $v_{\Tv}$ and computes the output $\hat{y}_{\text{DGN}}(x)=\ip{\phi_{\Tf}(x),v_{\Tv}}$.  The gating network is also called the feature network since it realises the neural path features, and the weight network is also called as the value network since it realises the neural path value.
