...
based on which it was concluded that learning in the gates (i.e., neural path features) is crucial for finite width CNN-GAP to outperform the infinite width CNTK-GAP. It was also shown that the DGN can be trained standalone and are only \emph{marginally poor} to the DNN as shown in \Cref{fig:dgn}. % this \textbf{explains the open question} in \Cref{sec:ntk}.
