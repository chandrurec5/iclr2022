...
\end{minipage}
\end{minipage}
\caption{\small{Shows the DGN on the left. In the case of fixed learnt gates, the feature network is pre-trained using $\hat{y}_f$ as the output, and the feature network is frozen. In the case of fixed random gates, the feature network is initialised at random, and frozen. In case of fixed gates hard-gating is used. In the case learnable gates both feature and value network are initialised at random and trained together, soft-gating is used to allow gradient flow through feature network. On the right side is the $4$ layer CNN-GAP and its DGN used in \cite{npk}. Here}}
\label{fig:dgn}
\end{figure}
